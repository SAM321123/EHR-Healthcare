...
import { CREATE_PAYMENT_INTENT, CREATE_SUBSCRIPTION_AFTER_CANCEL, GET_PRACTICE_DATA_SETTING } from 'src/store/types';
...
const MembershipFormForm = ({email, practitionerCount, rnCount, prescriberCount, cost, closeCreateSubscriptionModal, getSubscription}) => {
  ...
  useEffect(() => {
    if (!isEmpty(response)) {
      ...
      getSubscription({id: response?.practiceId });
    //   navigate(navigateTo(UI_ROUTES.login));
    }
  },[response, clearData, closeCreateSubscriptionModal, getSubscription]);
