...
import {  GET_SOCIAL_HISTORY,  UPDATE_SOCIAL_HISTORY } from 'src/store/types';
import palette from 'src/theme/palette';
import SocialHistoryFormGroups from './formGroups';
import LoadingButton from 'src/components/CustomButton/loadingButton';
import { dateFormatter, getUpdatedFieldsValue, showSnackbar } from 'src/lib/utils';
...
const SocialHistoryRendrer = ({historyDataItem={}}) => {
    const [mode,setMode] = useState('view');
    const [open,setOpen] = useState(false);
    const params = useParams();
    const {patientId}= params || {}
    const [response, , loading, callUpdateAPI, clearData] = useCRUD({
        id: `${UPDATE_SOCIAL_HISTORY}-${historyDataItem?.id}`,
        url: `${API_URL.socialHistory}/${historyDataItem?.id}`,
    ...
    const defaultValue= useMemo(()=>{
        return SocialHistoryFormGroups.reduce((acc,item)=>{
  ...
  return (
    ...
    {mode==='edit' &&   <div>
      <CustomForm
          form={form}
          formGroups={SocialHistoryFormGroups}
          columnsPerRow={1}
                ...
                <div style={{backgroundColor:'#F8F9FC',padding:12,display:'flex',justifyContent:'space-between',gap:82.25,borderRadius:4,alignItems:'center'}}>
                    <div style={{display:'flex',gap:10,alignItems:'center'}}>
                        <div style={{display:'flex',gap:10}}>
                            <div><Typography  style={{fontSize:12,fontWeight:500,lineHeight:'18px'}}>{`${historyDataItem?.socialHistory?.code==='other_social_history_code' ?`${historyDataItem?.socialHistoryOther} (${historyDataItem?.socialHistory?.name})`  : historyDataItem?.socialHistory?.name || `Tobacco`} `}</Typography></div>
                            <div><Typography  style={{fontSize:12,fontWeight:500,lineHeight:'18px'}}>{`${historyDataItem?.condition?.name || `Since ${dateFormatter(historyDataItem.date,dateFormats.MMDDYYYY)}`}`}</Typography></div>
                        </div>
...
export default SocialHistoryRendrer;
